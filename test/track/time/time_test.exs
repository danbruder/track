defmodule Track.TimeTest do
  ...
  # Time.create_client(Map.merge(@valid_attrs, %{user_id: user.id}))
  ...
  # Time.create_project(
  # Map.merge(@valid_attrs, %{user_id: user.id, client_id: client.id})
  # )
  ...
  ## Intentionally not testing computed fields. Tested below
  # end

  ## When the project is billable, 
  ##  - revenue is bill_rate * hours
  ##  - internal_cost = internal_rate * hours
  ##  - opportunity_cost = 0
  ##  - profit = revenue - internal_cost
  ## When the project is non-billable, 
  ##  - revenue is 0
  ##  - internal_cost = internal_rate * hours
  ##  - opportunity_cost = (bill_rate * hours - internal_rate * hours)
  ##  - profit = 0
  # test "create_log/1 calculates calculated fields properly" do
  # user = user_fixture()
  # client = client_fixture(%{user_id: user.id})
  # project = project_fixture(%{client_id: client.id, user_id: user.id})
  ...
  # assert Decimal.equal?(log.revenue, Decimal.new(1200.0))
  # assert Decimal.equal?(log.internal_cost, Decimal.new(600.0))
  # assert Decimal.equal?(log.opportunity_cost, Decimal.new(0))
  # assert Decimal.equal?(log.profit, Decimal.new(600.0))
  ...
  # {:ok, log2} = 
  # log_attrs2
  # |> Map.merge(%{billable: false})
  # |> Time.create_log()

  # assert Decimal.equal?(log2.revenue, Decimal.new(0.0))
  # assert Decimal.equal?(log2.internal_cost, Decimal.new(600.0))
  # assert Decimal.equal?(log2.opportunity_cost, Decimal.new(600.0))
  # assert Decimal.equal?(log2.profit, Decimal.new(600.0))
  # end
  ...
  ## assert log.revenue == Decimal.new("456.7")
